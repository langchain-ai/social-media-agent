...
import { xquikLoader } from "../loaders/xquik.js";
...
export async function ingestData(
  ...
  if (useLangChainPrompts()) {
    ...
    if (sources.includes("xquik")) {
      try {
        tweets = [
          ...tweets,
          ...(await xquikLoader({
            limit: config.configurable?.[XQUIK_SEARCH_LIMIT],
            searchQuery: config.configurable?.[XQUIK_SEARCH_QUERY],
          })),
        ];
      } catch (e) {
        console.error("Failed to load tweets with Xquik", e);
      }
    }
